...
def get_existing_comment_id():
	comment_id = -1
	comment_body = ""
	os.system('curl -s -H "Authorization: token ' + token + '" -H "Accept: application/vnd.github.groot-preview+json" https://api.github.com/repos/' + repo + '/issues/' + str(pr_number) + '/comments > temp.json')
...
if comment_id == -1:
	os.system('curl -s --location --request POST --header "Authorization: Bearer ' + token + '"  --header "Content-Type: application/json" https://api.github.com/repos/' + repo + '/issues/' + str(pr_number) + '/comments --data "{\\\"body\\\": \\\"' + new_message + '\\\"}"')
else:
	os.system('curl -s --location --request PATCH --header "Authorization: Bearer ' + token + '"  --header "Content-Type: application/json" https://api.github.com/repos/' + repo + '/issues/comments/' + str(comment_id) + ' --data "{\\\"body\\\": \\\"' + new_message + '\\\"}"')
